# jujjybez

<div align="center">

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=20&duration=3000&color=9966FF&center=true&vCenter=true&lines=Студент+ибас;Любитель+backend;Любитель+сертификации+ФСТЕК;C%2B%2B+%26+Python+разработчик)](https://git.io/typing-svg)

</div>

---

## Языки и инструменты

<div align="center">

<table>
  <tr>
    <td align="center">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="60" height="60"/><br>
      <b>C++</b>
    </td>
    <td align="center">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" width="60" height="60"/><br>
      <b>C#</b>
    </td>
    <td align="center">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="60" height="60"/><br>
      <b>Python</b>
    </td>
    <td align="center">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="60" height="60"/><br>
      <b>Linux</b>
    </td>
    <td align="center">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="60" height="60"/><br>
      <b>VS Code</b>
    </td>
    <td align="center">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/visualstudio/visualstudio-original.svg" width="60" height="60"/><br>
      <b>Visual Studio</b>
    </td>
    <td align="center">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pycharm/pycharm-original.svg" width="60" height="60"/><br>
      <b>PyCharm</b>
  </tr>
</table>

</div>

---

## Интересы и направления

- **Информационная безопасность:** Аудит, Kali Linux, системная безопасность
- **Нейронные сети:** Перцептрон, Hopfield-сети, Kohonen-сети, диффузные нейроны
- **Цифровая логика:** Логические схемы, булева алгебра, Proteus, Multisim
- **Математика:** Дискретная математика, алгебраисечкие системы, криптография
- **И еще много чего технического**

---

## Статистика GitHub

<div align="center">

### Общая статистика

![GitHub stats](https://github-readme-stats.vercel.app/api?username=jujjybez&show_icons=true&theme=radical&hide_border=true)

### Streak

![GitHub Streak](https://streak-stats.demolab.com/?user=jujjybez&theme=radical)

### ЛеetCode

[![LeetCode stats](https://leetcode-stats-six.vercel.app/api?username=jujjybez&theme=dark)](https://github.com/jujjybez/leetcode-stats)

</div>

---

## Распределение языков

<div align="center">

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=jujjybez&layout=compact)](https://github.com/jujjybez/github-readme-stats)

</div>

---

## Проекты

<table>
  <tr>
    <td>
      <h3>Логические схемы</h3>
      <p>Симуляция булевых функций и логических схем</p>
      <ul>
        <li>Proteus Simulation</li>
        <li>Boolean Functions</li>
        <li>Digital Logic Design</li>
      </ul>
      <strong>C++, Proteus, Multisim</strong>
    </td>
    <td>
      <h3>Нейронные сети</h3>
      <p>Реализация различных технологий НС</p>
      <ul>
        <li>Single-layer Perceptron</li>
        <li>Supervised and unsupervised Learning</li>
        <li>Recurrent Neural Network</li>
        <li>Content-addressable Memory</li>
      </ul>
      <strong>Python, NumPy</strong>
    </td>
  </tr>
  <tr>
    <td>
      <h3>Криптография</h3>
      <p>Алгоритмы шифрования и теория групп</p>
      <ul>
        <li>Group Theory Applications</li>
        <li>Encryption Algorithms</li>
      </ul>
      <strong>C++, Python</strong>
    </td>
    <td>
      <h3>Linux Config</h3>
      <p>Конфигурация и multitool</p>
      <ul>
        <li>Bootloader Setup</li>
        <li>Multi-boot Environment</li>
      </ul>
      <strong>Linux, Bash</strong>
    </td>
  </tr>
</table>

---

## Recent Activity


<p align="center">
  <img src="https://raw.githubusercontent.com/jujjybez/jujjybez/main/github-contribution-grid-snake.svg" alt="Snake Animation"/>
</p>


---

<div align="center">

### Спасибо за визит

</div>
